  ...
  <div>
    ...
    <div class="row">
      <div class="col-12 col-sm-6 text-center text-sm-left mb-4 mb-sm-0">
        <span class="text-uppercase page-subtitle"> {{$t("message.currency.e_commerce_setup")}}
        </span>
        <h3 class="page-title">{{$t("message.currency.currency")}}</h3>
      </div>
      <div class="col-12 col-sm-6 d-flex align-items-center">
        <div class="mx-auto ml-sm-auto mr-sm-0">
          <CButton block color="info" @click="openModal">{{$t("message.currency.add_new_currency")}}</CButton>
        </div>
      </div>
    </div>
    <!-- End Page Header -->
    ...
    <!-- Datatable -->
      <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer my-5">
        ...
        <div class="dataTables_length"><label>{{$t("message.currency.show")}}
          <select v-model="tableData.length" @change="loadData()">
            <option v-for="(records, index) in perPage" :key="index" :value="records">{{ records }}</option>
          </select>
          {{$t("message.currency.entries")}}</label></div>
        <div class="dataTables_filter">
          <label>{{$t("message.currency.search")}}
        ...
        <div class="dataTables_footer">
          <div class="dataTables_info">
            {{$t("message.currency.showing")}} {{ projects.from }} {{$t("message.currency.to")}} {{ projects.to }} {{$t("message.currency.of")}} {{ projects.total }} {{$t("message.currency.entries")}}
          </div>
          <div class="dataTables_paginate paging_simple_numbers">
            <pagination :data="projects" :show-disabled="true" :align="align" :limit="2"
                        @pagination-change-page="loadData">
              <span slot="prev-nav">{{$t("message.currency.previous")}}</span>
              <span slot="next-nav">{{$t("message.currency.next")}}</span>
            </pagination>
          </div>
        </div>
      </div>
    ...
    <!-- Modal -->
    <b-modal id="adminModal" :title="editMode ? this.$t('message.currency.currency_information_edit'): this.$t('message.currency.new_currency_add')"
      ...
      <b-form @submit.prevent="editMode ? updateCurrency(): createCurrency()" @keydown="form.onKeydown($event)">
        <CInput label="Currency Name :" v-model="$v.form.name.$model"
                ...
                :invalidFeedback="!$v.form.name.required? this.$t('message.currency.currency_name_required'): $v.form.name.maxLength?'': this.$t('message.currency.currency_name_character') "
        ...
        <CInput label="Currency Symbol:" v-model="$v.form.symbol.$model"
                ...
                :invalidFeedback="!$v.form.symbol.required? this.$t('message.currency.currency_symbol_required'): $v.form.symbol.maxLength?'': this.$t('message.currency.currency_symbol_character') "
        ...
        <CInput label="Currency Code :" v-model="$v.form.code.$model"
                ...
                :invalidFeedback="$v.form.code.maxLength?'': this.$t('message.currency.currency_code_character') "
        ...
        <CInput label="Exchange Rate :" v-model="$v.form.exchange_rate.$model"
                ...
                :invalidFeedback="$v.form.exchange_rate.maxLength?'':this.$t('message.currency.exchange_rate_character') "
        ...
        <CRow class="justify-content-end mt-5">
          <CCol col="4" sm="4" md="3" class="mb-3 mb-xl-0">
            <CButton block color="info" type="submit" :disabled="form.busy">
              {{ editMode ? this.$t("message.currency.update") : this.$t("message.currency.submit") }}
            </CButton>
          </CCol>
          <CCol col="4" sm="4" md="3" class="mb-3 mb-xl-0">
...
export default {
  ...
  methods: {
    ...
    createCurrency() {
      ...
      this.form.post('currency')
          .then((e) => {
            ...
            toast.fire({
              icon: 'success',
              title: this.$t("message.currency.currency_add_successfully")
            });
          })
          .catch((e) => {
    ...
    updateCurrency() {
      ...
      this.form.put('currency/' + this.form.id)
          .then(() => {
            ...
            toast.fire({
              icon: 'success',
              title: this.$t("message.currency.currency_update_successfully")
            });
          })
          .catch((e) => {
            swal(this.$t('message.common.error'), this.$t("message.common.something_wrong"), 'warning')
    ...
    deleteCurrency(id) {
      swal.fire({
        title: this.$t("message.currency.are_you_sure"),
        text: this.$t("message.currency.able_to_revert"),
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: this.$t("message.currency.delete_it")
      }).then((result) => {
        if (result.value) {
          this.form.delete('area/' + id)
              .then((data) => {
                ...
                toast.fire({
                  icon: 'success',
                  title: this.$t("message.currency.currency_deleted_successfully")
                });
              })
              .catch(() => {
                swal(this.$t('message.common.error'), this.$t("message.common.something_wrong"), 'warning')
